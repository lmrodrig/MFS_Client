 ...
 * ---------- ---- -------- ---------------- ----------------------------------
 * 2010-02-19      46810JL  J Mastachi       -Initial version.
 * 2010-04-01 ~01  48083JW  J Mastachi       -Fix for text fields not showing in 
 *                                            DBCS environments.
 * 2010-04-09 ~02  48519JW  J Mastachi       -Support new CRTWCARECS return codes.
...
/**
 * <code>MFSWarrantyCardPRCDialog</code> is displayed when the PRC Warranty
 * Card Mode button is selected on <code>MFSWarrantyCardModeDialog</code>.
 * @author The MFS Client Development Team
 */
public class MFSWarrantyCardPRCDialog extends MFSActionableDialog implements DocumentListener
{
	private static final long serialVersionUID = 1L;
	/** Return code from CRTWCARECS for no key parts found. */
	private final int NO_KEY_PARTS_FOUND = 3; //~02C
	
	/** Return code from CRTWCARECS for no FFBM description found. */ //~02A
	private final int NO_FDES_FOUND = 2; //~02A
	
	/** Return code from CRTWCARECS for no key parts and no FFBM description found. */ //~02A
	private final int NO_KEY_PARTS_NO_FDES_FOUND = 1; //~02A
	...
	/** The Warranty Card SN value <code>JLabel</code>. */
	private JLabel vlCardSN = null;
	...
	/** The <code>JList</code> that displays the key parts. */
	private JList lstKeyParts = createList();
	
	/** The <code>JScrollPane</code> that contains the key parts list. */
	private JScrollPane spKeyParts = new JScrollPane(this.lstKeyParts,
			ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
			ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	
	/** The <code>ListModel</code> for {@link #lstKeyParts}. */
	private DefaultListModel lmKeyParts = new DefaultListModel();
	
	/** The <code>JPanel</code> for the key parts list section */
	private JPanel keyPartPanel = new JPanel();
	
	/** The Add <code>JButton</code> for the key parts list. */
	private JButton pbAdd = createButton("Add", 'A');
	
	/** The Remove <code>JButton</code> for the key parts list. */
	private JButton pbRemove = createButton("Remove", 'M');
	...
	/** Set <code>true</code> if the Warranty Card SN is decoded. */
	private boolean cardSNfound = false;
	...
	/**
	 * Constructs a new <code>MFSWarrantyCardPRCDialog</code>.
	 * @param parent the <code>MFSFrame</code> that caused this
	 *        <code>MFSWarrantyCardPRCDialog</code> to be displayed
	 */
	public MFSWarrantyCardPRCDialog(MFSFrame parent)
	{
		super(parent, "PRC Warranty Card Mode");
		...
		this.lstKeyParts.setModel(this.lmKeyParts);
		this.keyPartPanel.setVisible(false);
		createLayout();
		addMyListeners();
	}
	...
	/** Adds listeners for Add and Remove buttons in Key Parts section.*/
	private void addKeyPartListeners()
	{
	...
	/** Removes listeners for Add and Remove buttons in Key Parts section.*/
	private void removeKeyPartListeners()
	{
		this.pbAdd.removeActionListener(this);
		this.pbAdd.removeKeyListener(this);
		this.pbRemove.removeActionListener(this);
		this.pbRemove.removeKeyListener(this);
	}
	...
	private void createLayout()
	{
		...
		this.lstKeyParts.setVisibleRowCount(3);
		
		JLabel titleLabel = createLabel("PRC Data Entry Mode");
		...
		JLabel cardSerLabel = createSmallNameLabel("Warranty Card SN:");
		if (this.vlCardSN == null)
		{
			this.vlCardSN = createValueLabel(cardSerLabel);
		}
		...
		JLabel keyPartLabel = createLabel("Key Parts");
		keyPartLabel.setForeground(MFSConstants.PRIMARY_FOREGROUND_HIGHLIGHT_COLOR);
		keyPartLabel.setFont(MFSConstants.MEDIUM_DIALOG_FONT);
		...
		gbcCard.insets = new Insets(5, 10, 0, 5);
		...
		gbcCard.gridy++;
		gbcCard.insets.bottom = 5;
		cardPanel.add(cardSerLabel, gbcCard);
		
		gbcCard.gridx = 1;
		gbcCard.insets = new Insets(5, 5, 0, 10);
		...
		gbcCard.gridy++;
		gbcCard.insets.bottom = 5;
		cardPanel.add(this.vlCardSN, gbcCard);
		...
		gbcFFBM.insets = new Insets(5, 10, 0, 5);
		...
		gbcFFBM.gridy++;
		gbcFFBM.insets.bottom = 5;
		ffbmPanel.add(mcsnLabel, gbcFFBM);      //~04C
		
		gbcFFBM.gridx = 1;
		gbcFFBM.insets = new Insets(5, 5, 0, 10);
		...
		this.keyPartPanel.removeAll();
		this.keyPartPanel.setLayout(new GridBagLayout());
		this.keyPartPanel.setBackground(MFSConstants.PRIMARY_BACKGROUND_COLOR);
		this.keyPartPanel.setBorder(blueline);
		
		GridBagConstraints gbcKeyPart = new GridBagConstraints();
		gbcKeyPart.insets = new Insets(5, 10, 0, 10);
		
		gbcKeyPart.gridy = 0;
		gbcKeyPart.gridx = 0;
		gbcKeyPart.gridwidth = 2;
		this.keyPartPanel.add(keyPartLabel, gbcKeyPart);
		gbcKeyPart.gridy++;
		this.keyPartPanel.add(this.spKeyParts, gbcKeyPart);
		gbcKeyPart.gridy++;
		gbcKeyPart.gridwidth = 1;
		gbcKeyPart.insets.bottom = 5;
		gbcKeyPart.insets.right = 5;
		gbcKeyPart.anchor = GridBagConstraints.EAST;
		this.keyPartPanel.add(this.pbAdd, gbcKeyPart);
		gbcKeyPart.gridx = 1;
		gbcKeyPart.insets.right = 10;
		gbcKeyPart.insets.left = 5;
		gbcKeyPart.anchor = GridBagConstraints.WEST;
		this.keyPartPanel.add(this.pbRemove, gbcKeyPart);
		...
		gbc.gridy++;
		contentPane.add(this.keyPartPanel, gbc);
	...
	private void print()
	{
		...
		try
		{
			...
			else if (!this.cardSNfound)
			{
				this.errorLabel.setText("Warranty Card Serial Number is a required field.");
				focus_on_tf_card = true;
				rc = 1;
			}
			...
			if (rc == 0)
			{
				...
				crtwcarecs.addElement("WCSN", this.vlCardSN.getText().toUpperCase().trim());
				...
				if (!this.lmKeyParts.isEmpty())
				{
					ArrayList<StringBuffer> al = new ArrayList<StringBuffer>();
					for (int i = 0; i < this.lmKeyParts.size(); i++)
					{
						String row = (String) this.lmKeyParts.get(i);
						StringBuffer keypart = new StringBuffer("<INPN>");
						keypart.append(row.substring(3, 15));
						keypart.append("</INPN><INSQ>");
						keypart.append(row.substring(19));
						keypart.append("</INSQ>");
						al.add(keypart);
					}
					Iterator itr = al.iterator();
					crtwcarecs.addElements(itr, "KEYPARTS", "KEYPART");
				}
				else
				{
				...
				crtwcarecs.addElement("DEMD", "PRC");
				...
				if (rc == 0)
				{
					...
					if (rc == 0) //~03A
					{
						...
						label_data.append("</WCPN>");
						label_data.append("<WCSN>");
						label_data.append(this.vlCardSN.getText().toUpperCase().trim());
						label_data.append("</WCSN>");
						...
						label_data.append("</USER>");
						label_data.append("<DEMD>PRC</DEMD>");

						label_trigger.append("<PRCOPTWNTY>");
						...
						else
						{
							...
							if (plom_file.exists())
							{
								...
								try
								{
									retval = plom_parser.getFieldOnly("PLOMPRC");
								}
								...
								try
								{
									jpnplom = plom_parser.getFieldOnly("PLOMJPN");
								}
								catch (MISSING_XML_TAG_EXCEPTION e)
								{
									jpnplom = "";
							...
							if (retval.length() == 3 && saveplomfile == true)
							{
								try
								{
									...
									out.println("<PLOMPRC>"+retval+"</PLOMPRC>");
									out.println("<PLOMJPN>"+jpnplom+"</PLOMJPN>");
									out.println("</LASTPLOM>");
									out.close();
								}
								catch (Exception e)
				...
				else if (rc == this.NO_KEY_PARTS_NO_FDES_FOUND) //~02C
				{
					String message = "No Key Parts and no FFBM Description were found. Would you like to enter them manually?"; //~05A
					...
					if(parser.fieldExists("FDES")) //~05A
					{
						...
						message = "No Key Parts were found. Would you like to enter them manually?"; //~05A
						this.focus_on_tf_desc = false; //~05A
					}
					else
					{
						this.focus_on_tf_desc = true;  //~04C
					...
					if (IGSMessageBox.showYesNoMB(this, null, message, null)) //~02C
					{
						MFSKeyPartEntryDialog myKPED = new MFSKeyPartEntryDialog(this.lmKeyParts, getParentFrame());
						myKPED.setLocationRelativeTo(getParentFrame());
						myKPED.setVisible(true);

						this.keyPartPanel.setVisible(true);
						this.tfFFBMDesc.setVisible(this.focus_on_tf_desc); //~05C
					...
					{
						this.rcvdMATPMMDL = parser.getField("MATPMMDL").trim();
				...
				else if (rc == this.NO_KEY_PARTS_FOUND) //~02A BEGIN
				{
					if (IGSMessageBox.showYesNoMB(this, null, "No Key Parts were found. Would you like to enter them manually?", null))
					{
						MFSKeyPartEntryDialog myKPED = new MFSKeyPartEntryDialog(this.lmKeyParts, getParentFrame());
						myKPED.setLocationRelativeTo(getParentFrame());
						myKPED.setVisible(true);
						...
						MfsXMLParser parser = new MfsXMLParser(crtwcarecs.getOutput());
						if (parser.fieldExists("MATPMMDL"))
						{
							this.rcvdMATPMMDL = parser.getField("MATPMMDL").trim();
							this.tfMATPMMDL.setText(rcvdMATPMMDL);
							this.tfMCSN.setText("");
				...
				else if (rc == this.NO_FDES_FOUND)
				{
					//~05C BEGIN
					MfsXMLParser parser = new MfsXMLParser(crtwcarecs.getOutput());

					if(parser.fieldExists("FDES"))
					{
						this.tfFFBMDesc.setText(parser.getFieldOnly("FDES").trim());
					}
					else if (IGSMessageBox.showYesNoMB(this, null, "No FFBM Description was found. Would you like to enter it manually?", null))
					{
	...
	private void validateInput()
	{
		...
		if (!this.vlCardSN.getText().equals(""))
		{
			this.cardSNfound = true;
		}
		...
		if (this.cardPNfound && this.cardSNfound && this.ffbmPNfound && this.ffbmSNfound)
		{
			print();  //~04C
		}
	}
	...
	private void clear()
	{
		...
		this.tfMCSN.setVisible(false);
		this.tfMCSN.setEnabled(false);
		this.lmKeyParts.clear();
		this.keyPartPanel.setVisible(false);
		removeKeyPartListeners();
		this.cardPNfound = false;
		this.cardSNfound = false;
	...
	public void actionPerformed(ActionEvent ae)
	{
		try
		{
			...
			{
				this.errorLabel.setText("");  //~04A
				if (IGSMessageBox.showYesNoMB(this, null, "Are you sure you want to remove the selected key part?", null))
				{
					int index = this.lstKeyParts.getSelectedIndex();
					if (index == -1)
					{
						this.errorLabel.setText("Please select a Key Part to be removed.");
						createLayout();
					}
					else
					{
						this.lmKeyParts.remove(index);
					}
				}
			}
		}
	...
	public void keyPressed(KeyEvent ke)
	{
		...
		if (keyCode == KeyEvent.VK_ENTER)
		{
			...
			{
				recvInput(this.tfCard, this.vlCardPN, this.vlCardSN);
				...
				else if (this.cardPNfound && this.cardSNfound)  //~04C
				{
					this.tfFFBM.requestFocusInWindow();
				}
				else
