/* @Copyright IBM Corporation 2004, 2016. All rights reserved.
 ...
 * ---------- ---- -------- ---------------- ----------------------------------
 *              ~1 248441DF                  -make sure activeRow is set when we are looking for the next part to remove
 *                                           -make sure the cursor gets set back to the default cursor
 * 2004-04-07      26665KM                   -Changed keyPressed method for:
 *                                            scrap and ncm keys allowed even though not displayed on screen
 * 2005-09-24   ~2 32680JM                   -DEFECT in PREP
 *                                           -allow the user to take out a 0 quantity part with 'R' idsp for PREP
 * 2006-06-14   ~3 31801JM  R Prechel        -Removed unnecessary methods, variables, and imports
 *                                           -Changed parent class to MFSLogPartDialog;
 *                                            inherited methods previously defined locally
 * 2007-02-27   ~4 34242JR  R Prechel        -Java 5 version
 * 2007-03-28   ~5 19622JM  R Prechel        -Remove CHCK_PLUG transaction call
 *                                           -Show a message when a Loaner part is removed
 *                                           -Delete downlevel check to allow the PF9 NCM function
 *                                            even if the part will be NCM'd
 *                                           -Print the true defect of a part being automatically
 *                                            NCM'd by deconfig
 *                                           -No missing or kept buttons for assemblies (typz of ' ')
 * 2007-04-02   ~6 38166JM  R Prechel        -Add setLocationRelativeTo for dialog
 * 2007-05-14   ~7 38421KM  R Prechel        -Call NCM logic automatically when a part is removed
 *                                            and the Plug exceeded flag is true
 * 2007-09-27   ~8 39988KM  R Prechel        -Pass MCTL to RTV_DRPDWN
 * 2008-01-14      39782JM	Martha Luna		 -Changes the name of updtMultilineDisplayString by updtIRDisplayString
 * 2009-07-10  ~09 45703EM  Christopher O    -Call MFSPartDataCollectDialog in logPart() if is required
 * 2010-11-02  ~10 49513JM  Toribio H.       -Improve Label Updates of scanned values 
 * 2014-12-18  ~11 Story1262255 VH Avila     -Make sure that always set the active row after any process
 ******************************************************************************/
...
/**
 * <code>MFSLogDeconfigPartDialog</code> is the
 * <code>MFSAbstractLogPartDialog</code> used by the
 * {@link MFSDeconfigPanel#removepn(String)} method.
 * @author The MFS Client Development Team
 */
@SuppressWarnings("serial")
public class MFSLogDeconfigPartDialog extends MFSAbstractLogPartDialog
{
	//~4A Added log type constants.
	/** The Remove log type. */
	...
	/** The Missing (F6) <code>JButton</code>. */
	private JButton pbMissing = createButton("F6 = Missing"); //$NON-NLS-1$

	/** The Kept (F8) <code>JButton</code>. */
	private JButton pbKept = createButton("F8 = Kept"); //$NON-NLS-1$
	...
	/** The <code>MFSDeconfigPanel</code> that uses this dialog. */
	private MFSDeconfigPanel fieldDeconfig; //set by constructor
	...
	private void createLayout()
	{
		...
		//~2  No missing or kept buttons for the PREP process
		//~5A No missing or kept buttons for assemblies (typz of ' ')
		if (prep || this.fieldHeaderRec.getTypz().equals(" ")) //$NON-NLS-1$
		{
			this.pbMissing.setEnabled(false);
			this.pbKept.setEnabled(false);
		}
		else
		{
			this.pnlButtons.add(this.pbMissing);
			this.pnlButtons.add(this.pbKept);
			this.pbMissing.setEnabled(true);
			this.pbKept.setEnabled(true);
		}

		if (config.getConfigValue("SCRP").equals("Y") && !prep) //$NON-NLS-1$ //$NON-NLS-2$
		{
	...
	private void addMyListeners()
	{
		...
		this.pbMissing.addActionListener(this);
		this.pbKept.addActionListener(this);
		this.pbScrap.addActionListener(this);
		this.pbNCM.addActionListener(this);

		this.pbProcess.addKeyListener(this);
		this.pbCancel.addKeyListener(this);
		this.pbMissing.addKeyListener(this);
		this.pbKept.addKeyListener(this);
	...
	/** Invoked when {@link #pbKept} is selected. */
	private void customerKept()
	{
		try
		{
			int rc = 0;
			int qt = 1;
			boolean overrideQty = false;
			MFSComponentRec compRec = (MFSComponentRec) this.fieldJList.getSelectedValue();
			String errorString = EMPTY_STRING;

			JLabel qtyLabel = this.getValueLabel(PART_QUANTITY, 1);
			if (qtyLabel != null)
			{
				qt = Integer.parseInt(qtyLabel.getText());
				overrideQty = true;
			}
	...
	private void handleInstruction(MFSPartInstructionJPanel pip)
	{
		try
		{
			...
			hideMode=this.fieldDeconfig.isHideMode();
			this.fieldDeconfig.scrollToPip(pip);
	...
	/** Invoked when {@link #pbMissing} is selected. */
	private void missingPart()
	{
		try
		{
			final MFSConfig config = MFSConfig.getInstance();
			...
			JLabel qtyLabel = this.getValueLabel(PART_QUANTITY, 1); //~10
			if (qtyLabel != null)
			{
				qt = Integer.parseInt(qtyLabel.getText());
				overrideQty = true;
	...
	protected void recvInput()
	{
		try
		{
			...
			{
				barcode.setMyBCPartObject(new MFSBCPartObject());
				rc = decodeBarcode(this.fieldHeaderRec, compRec, barcode); //~4C
			}
			found = noInputToCollect(compRec, barcode.getBCMyPartObject()); //~4C
			if (rc == 0 && !found)
			{
				/* QT */
				if (!barcode.getBCMyPartObject().getQT().equals(EMPTY_STRING))
				{		
					if (null == this.getValueLabel(PART_QUANTITY, 1)) // ~10
					{
				...
				{
					found |= setValueLabelText(PART_NUMBER, 2, barcode.getBCMyPartObject().getPN()); //~4C ~10
				...
				{
					found |= setValueLabelText(EC_NUMBER, 3, barcode.getBCMyPartObject().getEC()); //~4C ~10
				}
				/* SN */
				if (!(barcode.getBCMyPartObject().getSN().equals(EMPTY_STRING)))
				{
					if (validateSequenceNumber(compRec, barcode.getBCMyPartObject())) {
						found |= setValueLabelText(SEQUENCE_NUMBER, 4, barcode.getBCMyPartObject().getSN()); //~4C ~10	
					}					
				}
				/* CA */
				if (!(barcode.getBCMyPartObject().getCA().equals(EMPTY_STRING)))
				{
					found |= setValueLabelText(CARD_ASSEMBLY, 5, barcode.getBCMyPartObject().getCA()); //~4C ~10					
				}
				/* MS */
				if (!(barcode.getBCMyPartObject().getMSN().equals(EMPTY_STRING)))
				{
					JLabel msLabel = getValueLabel(MACHINE_SERIAL, 5); //~10
					if (null != msLabel) //~10
					{
						if (validateMachineSerial(compRec, barcode.getBCMyPartObject())) {//~4C						
							VRFY_PI vrfyPI = new VRFY_PI(this); //~10
							...
							{
								msLabel.setText(barcode.getBCMyPartObject().getMSN()); //~10
								found = true; 
							}
						}
					}
				...
				if (!(barcode.getBCMyPartObject().getWU().equals(EMPTY_STRING)))
				{
					if (!compRec.isCooiDoNotCollect())
					{
						RTV_SIGPN rtvSigPN = new RTV_SIGPN(this); //~10
					...
					if (rc == 0)
					{
						found |= setValueLabelText(CONTROL_NUMBER, 6, barcode.getBCMyPartObject().getWU()); //~4C ~10 
					} // end good return code from transaction
				}
				/* CO */
				if (!(barcode.getBCMyPartObject().getCO().equals(EMPTY_STRING)))
				{
					found |= setValueLabelText(COUNTRY, 7, barcode.getBCMyPartObject().getCO()); //~4C ~10
				}
			}
		...
		try
		{
			...
			if (rc == 0)
			{
				...
				if (rc == 0)
				{
					...
					// update instruction for this row - update to partial completion or total completion
					handleInstruction(getRowVectorElementAt(this.fieldActiveRow));
					int row = this.fieldActiveRow;
	...
	public void actionPerformed(ActionEvent ae)
	{
		try
		{
			...
			else if (source == this.pbMissing)
			{
				missingPart();
			}
			else if (source == this.pbKept)
			{
				customerKept();
			}
	...
	public void keyPressed(KeyEvent ke)
	{
		...
		{
			this.pbMissing.requestFocusInWindow();
			this.pbMissing.doClick();
		}
		else if (keyCode == KeyEvent.VK_F8)
		{
			this.pbKept.requestFocusInWindow();
			this.pbKept.doClick();
		}
