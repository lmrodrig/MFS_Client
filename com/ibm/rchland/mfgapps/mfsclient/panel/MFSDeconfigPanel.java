 ...
 * 2007-03-12   ~3 34242JR  R Prechel        -Java 5 version
 * 2007-03-28   ~4 19622JM  R Prechel        -Only allow Extra part process if the EXDCF variable is "Y"
 * 2007-04-02   ~5 38166JM  R Prechel        -Add setLocationRelativeTo for dialogs
 * 2007-04-09   ~6 38176JM  R Prechel        -Fix MFS Client hang
 * 2007-04-14   ~7 38346KM  R Prechel        -Don't set INPN to blanks in new MFSComponentRec after splitting a part.
 *                                           -Set selected index after adding PIP to pnlRowHolder
 * 2008-01-14      39782JM	Martha Luna		 -Change name of updtMultilineDisplayString to updtIRDisplayString
 * 2008-02-20   ~8 37616JL  R Prechel        -Change button images.
 * 2010-03-09   ~9 47596MZ  D Kloepping      -EFFICIENCYON config present, do not lookup suspension
 * 											   codes default value.
 * 2010-11-01  ~10 49513JM	Toribio H.   	 -Make RTV_IQ Cacheable
 ******************************************************************************/
...
/**
 * <code>MFSDeconfigPanel</code> is the <code>MFSInstructionsPanel</code> used to
 * view <code>MFSPartInstructionJPanel</code>s for deconfig and reapply prep.
 * @author The MFS Client Development Team
 */
@SuppressWarnings("serial")
public class MFSDeconfigPanel
	extends MFSInstructionsPanel
	implements MouseListener
{
	/** The "Deconfig" screen name of a <code>MFSSelectWorkPanel</code>. */
	public static final String DECONFIG_SCREEN_NAME = "Deconfig"; //$NON-NLS-1$
	...
	private MFSMenuButton pbSearchRemove = MFSMenuButton.createSmallButton("Srch Rm", //$NON-NLS-1$
			"smF9.gif", null, "BUTTON,DECONFIG,SEARCHREM"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private MFSMenuButton pbRemPart = MFSMenuButton.createSmallButton("Rem P/N", //$NON-NLS-1$
			"smF11.gif", null, "BUTTON,DECONFIG,REMPART"); //$NON-NLS-1$ //$NON-NLS-2$

	/** The Split (SHFT + F1) <code>MFSMenuButton</code>. */
	private MFSMenuButton pbSplit = MFSMenuButton.createSmallButton("Split", //$NON-NLS-1$
			"smF13.gif", null, "BUTTON,DECONFIG,SPLITPART"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private MFSMenuButton pbTagReprint = MFSMenuButton.createSmallButton("Tag", //$NON-NLS-1$
			"smF15.gif", null, "BUTTON,DECONFIG,REPRINTTAG"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private MFSMenuButton pbDownListReprint = MFSMenuButton.createSmallButton("Dwn List", //$NON-NLS-1$
			"smF16.gif", null, "BUTTON,DECONFIG,REPRINTLIST"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private MFSMenuButton pbChangeStatus = MFSMenuButton.createSmallButton("Chg Sts", //$NON-NLS-1$
			"smF17.gif", null, "BUTTON,DECONFIG,CHANGESTATUS"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private MFSMenuButton pbNCMReprint = MFSMenuButton.createSmallButton("NCM Tag", //$NON-NLS-1$
			"smF18.gif", null, "BUTTON,DECONFIG,NCMREPRINT"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	/**
	 * Constructs a new <code>MFSDeconfigPanel</code>.
	 ...
	 */
	public MFSDeconfigPanel(MFSFrame parent, MFSPanel source, String screenName)
	{
	...
	{
		//~3C Redone to use button iterator
	...
	{
		//~3C Redone to use MFSMenuButton.isActive
		...
		if (config.containsConfigEntry("BUTTON,DECONFIG,EXTRAPART") //$NON-NLS-1$
		...
		if (config.containsConfigEntry("BUTTON,DECONFIG,PROCALL") //$NON-NLS-1$
	...
	private MFSButtonIterator createMenuButtonIterator()
	{
		...
		//~8C Put pbProcAll in correct order
	...
	public void end_work()
	{
		...
		try
		{
			...
			else
			{
				...
				{
					RTV_IQ rtvIQ = new RTV_IQ(this); //~10
	...
	public int end_wu(String ecod, String qcod, String oemo)
	{
		...
		try
		{
			...
			//all but PREP use END_VAL - 25900JM
			if (!this.fieldHeaderRec.getNmbr().equals("PREP")) //$NON-NLS-1$
			{
				endData.append("END_VAL   "); //$NON-NLS-1$
				endData.append(ecod);
				endData.append(qcod);
			...
			{
				endData.append("END_PREP  "); //$NON-NLS-1$
				endData.append(ecod);
				endData.append(qcod);
				endData.append(config.get8CharCellType());
				endData.append(config.get8CharCell());
			...
			if (n == JOptionPane.YES_OPTION)
			{
				...
				while (index < this.fieldComponentListModel.size())
				{
					...
					if (!next.getIdsp().equals("T")) //$NON-NLS-1$
					{
						...
						//put in for Pemstar Deconfig lady (Nancy Smith)
	...
	public void removepn(String logType)
	{
		try
		{
			...
			if (found)
			{
				...
				MFSLogDeconfigPartDialog myLogDeconfigPartD = new MFSLogDeconfigPartDialog(
	...
	//~3C Rename of fireHandleRestorePrevScreen
	/**
	 ...
	 * JList we are adding to the screen. We are inside a function that is
	 * called from DeconfigJPanel.extra_part() - which removes listeners and
	 * then adds back in, so we are between removes and adds, so if we add one
	 * now, we'll get a second one added for this JList when addMyListeners gets
	 * called at the end of extra_part
	 */
	/**
...
//~7D					newCr.setInpn("            "); //$NON-NLS-1$
	...
	public void actionPerformed(ActionEvent ae)
	{
		try
		{
			...
			{
				removepn(MFSLogDeconfigPartDialog.LT_REMOVE);
			}
			...
			{
				removepn(MFSLogDeconfigPartDialog.LT_SEARCHREMOVE);
			}
